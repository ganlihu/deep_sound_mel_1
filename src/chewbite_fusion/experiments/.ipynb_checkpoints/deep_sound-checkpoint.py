...
def deep_sound():
    ...
    class NaNMonitor(tf.keras.callbacks.Callback):
        def on_batch_end(self, batch, logs=None):
            ...
            # 每10批次检查一次输入数据（额外保险）
            if batch % 10 == 0 and hasattr(self.model, 'train_function'):
                try:
                    batch_x, batch_y = self.model.train_function.inputs[0].numpy(), self.model.train_function.inputs[1].numpy()
                    if np.isnan(batch_x).any():
                        logger.error(f"批次 {batch} 输入数据包含NaN！")
                        self.model.stop_training = True
                except:
                    pass
